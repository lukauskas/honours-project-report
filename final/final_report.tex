...
\todo{Fit info  about strand directionality somewhere}

\begin{figure}[p]
   \centering
   \copyrightbox[b]{\includegraphics[width=1.1\textwidth]{figures/background/pierce_11_5_FULL.jpg}}
   {Copyright 2005 W. H. Freeman Pierce, Benjamin. Genetics: A Conceptual Approach, 2nd ed. (New York: W. H. Freeman and Company), 292. Sublicensed by Scitable to be used for non-commercial use in an educational environment in print form (via \cite{Annunziato:2008wh}).}
...
the DNA sequence of a human diploid cell will take up around 2 meters, if stretched, yet it lies there, packaged into the volume as small as the nucleus of a cell. The proteins that lie within the nucleus are directly responsible for packaging the DNA. These proteins, called histones, fold the DNA around them, eventually wrapping it into three dimensional structures.

More specifically, DNA wraps around octomer of two copies of each of the four core histones, H2A, H2B, H3, and H4, creating a structure resembling beads on a string (as pictured in panels 2 and 3 of \autoref{fig:background:dna_packaging}). The fifth histone, H1, attaches to the DNA from the other side of the other histones (pictured in yellow, panel 4 of \autoref{fig:background:dna_packaging}). This DNA is then wrapped to a fiber, which is then subsequently compressed in some higher-order structures, as seen in \autoref{fig:background:dna_packaging}.

\section{Transcription and Translation}
\begin{figure}[t,b,p]
\centering
\copyrightbox[b]{
...
In order to understand how the genetic code is able to describe organisms, it is important to understand how the information flows within a living organism. The central dogma of molecular biology states \emph{DNA makes RNA makes protein [and proteins make us]}. In it's essence, it encodes the two key processes occurring within a living cell, transcription from DNA to RNA and translation from RNA to protein.

During transcription, RNA polymerase molecule binds to a region of DNA, indicated by a complex ensemble of proteins that bind to promoter regions of DNA. It then proceeds to read the information encoded in one of the strands. As it progresses through the genome, it creates a single-stranded RNA molecule that is complimentary to the DNA that it has just read. The parts of the genome that can be transcribed are known as \emph{genes}.

Eukaryotic genes typically consist of parts that are translated to proteins, exons, and the parts that are not, introns. When the gene is transcribed, the RNA for both the exons and introns is created. However, intron regions are then removed post-transcriptionally using a process known as RNA splicing (pictured in \autoref{fig:background:splicing}). The remaining regions are joined together and translated into a protein within a ribosome.

\section{Epigenetics}

The central dogma of molecular biology explains how living organisms are built from the genetic code.
However, if all cells of a living organism contain the same DNA sequence, how can the diversity of cell types within a human body be explained? Epigenetics is a field of genetics that studies such diversity and changes in gene expression, that occur without any changes in the DNA sequence.

It is known that the physical structure of DNA plays an important part in gene regulation. 
Tightly packed regions of DNA are harder to interact with, therefore harder to initiate the transcription with. Modifications of the core histones, e.g. acetylation, methylation or phosphation can change the underlying structure of chromatin and thus make the DNA more accessible for interactions.
