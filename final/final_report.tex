...
\todo{finish this, and maybe update with the results form running the experiment again}.
...
\begin{figure}[p]
   \centering
   \forloop{cluster_number}{1}{\value{cluster_number} < 14}{
        \begin{subfigure}[b]{0.13\textwidth}
            \centering
            \includegraphics[width=\textwidth]{figures/evaluation/tss_regions/cluster-\arabic{cluster_number}.pdf}
           \caption{}
           \label{fig:evaluation:tss_regions:prototypes:\arabic{cluster_number}}
        \end{subfigure}
        ~
   }
   \caption{Prototypes The thirteen clusters resulting from the hierarchical clustering in \autoref{fig:evaluation:tss_regions:cut}. Cluster (c) is probably closest to the
   bimodal pattern seen on average. Other prototypes seem to show an intriguing set of patterns.
   For instance, a the bimodal pattern containing a variety of small throughs can be seen in (a), probably resulting from longer regions of the data that have more than a single TSS in the proximity. 
   Clusters (b) and (e) seem to capture a set of regions that are not activated at the annotated transcription start sites, but are rather active in one of the edges of the window. Cluster (k) captures regions around transcription start sites that have well-documented bimodal peaks, yet the peak on one of the sides stretches much longer than the other. Heatmaps of regions that form each of these clusters can be seen in Figures \ref{fig:evaluation:tss_regions:heatmaps:unwarped} and \ref{fig:evaluation:tss_regions:heatmaps:warped}}
...
\begin{figure}
   ...
   \forloop{cluster_number}{1}{\value{cluster_number} < 14}{
        \begin{subfigure}[b]{0.3\textwidth}
            \centering
            \includegraphics[width=\textwidth]{figures/evaluation/tss_regions/unwarped-heatmaps/cluster-\arabic{cluster_number}.pdf}
           \caption{}
           \label{fig:evaluation:tss_regions:heatmaps:unwarped:\arabic{cluster_number}}
        \end{subfigure}
        ~
   }
   \caption{Regions forming the thirteen clusters resulting from the hierarchical clustering in \autoref{fig:evaluation:tss_regions:cut}. Actual, unwarped, data is pictured in this figure. As you can see the patterns within the data can be seen without the warping, yet these patterns become unmistakable after the warping, as seen in \autoref{fig:evaluation:tss_regions:heatmaps:warped}. Note that some clusters, e.g. clusters (g) and (h), have a visible division of the elements inside them, suggesting that cutting the dendrogram at a lower threshold, might capture the specifics of these sub-clusters better.}
...
\begin{figure}
   ...
   \forloop{cluster_number}{1}{\value{cluster_number} < 14}{
        \begin{subfigure}[b]{0.3\textwidth}
            \centering
            \includegraphics[width=\textwidth]{figures/evaluation/tss_regions/cluster-warped-\arabic{cluster_number}.pdf}
           \caption{}
           \label{fig:evaluation:tss_regions:heatmaps:warped:\arabic{cluster_number}}
        \end{subfigure}
        ~
   }
