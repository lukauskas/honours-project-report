...
that satisfy $|i-j| <= k$ where $k$ is some integer specifying the tightness of the constraint. It is commonly referred to as "Sakoe \& Chiba band" as it looks like a band around the main diagonal (see \autoref{fig:DTW:sakoe_chiba:cost}). Intuitively, this constraint imposes the requirement that each point on sequence A is mapped to a point no more than $k$ points away on the sequence B. That is, it assumes that there won't be any warping of the time axis greater than by $k$ points. Note that setting the $k$ parameter to zero would give us Euclidean distance between the two sequences.
