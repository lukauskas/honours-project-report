...
def visualise_dtw(index_a, index_b, *args, **kwargs):
    a = data.ix[index_a]
    b = data.ix[index_b]
...
def to_figure(index_a, index_b, extension='eps', *args, **kwargs):
    f = visualise_dtw(index_a, index_b, *args, **kwargs)
    f.savefig("{0}-{1}.{2}".format(index_a, index_b, extension))
    plt.close(f)
