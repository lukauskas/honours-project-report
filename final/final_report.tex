...
\begin{figure}
    ...
    \caption{Illustration of parallel sequencing of nucleotides, as in Illumina Sequencing platform.
...
the next-generation sequencing methods sequence a lot of short DNA segments in a massively-parallel way, much like as depicted in \autoref{fig:background:ngs:illumina}, where the sequencing step of Illumina/Solexa Sequencing Platform is illustrated as described in \cite{Metzker:j6NWuwFp}. 
...
Chromatin Immunoprecipitation, followed by Sequencing (ChIP-Seq) is a NGS method that studies protein-DNA interactions within the cell. Instead of sequencing the whole genome, ChIP-Seq aims to sequence only parts of DNA that interact with some target protein. These target proteins can either be internal to DNA (e.g. Histones) or arbitrary external proteins that interact with DNA.

Basic ChIP-Seq workflow is summarised in \autoref{fig:background:chip}. In short, proteins are first "glued" together with the DNA by chemical cross-linking. This DNA-with proteins attached to it is then shorn into smaller pieces and antibodies are added to single the target proteins out using immunoprecipitation. The precipitate is then collected in order to sequence the DNA that is attached to the target proteins\cite{Mardis:2007wa}. This sequenced DNA is then matched to the reference genome.
...
The aligned reads from the experiment are pictured as rectangles below the histogram. Since ChIP-Seq DNA fragments are usually sequenced from $5'$ end only\cite{Park:2009wc}, the reads have varying directionalities, depending on which strand was sequenced. These directionalities are indicated by the colour of the rectangle in the figure. These aligned reads are then stacked together to generate a
landscape of target protein enrichment, seen as the histogram in the figure \ref{fig:background:chip-results}.
