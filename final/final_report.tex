...
\begin{figure}[t,b]
   ...
   \caption{Illustration of best-case and worst-case linkage matrices that may result from clustering of eight items. The number above the horizontal nodes corresponds to the num
   ~
   \begin{subfigure}[b]{0.45\textwidth}
       \centering
   ...
   \end{subfigure}
   ber of items in the cluster. The number on the right side shows the number of items per level.}
\end{figure}

In order to compute the warped dataset, DTW projection algorithm has to be run for each of the possible cluster prototypes. This sounds like a tedious task, and indeed, in the worst case,  where the nodes would form a structure pictured in \autoref{fig:implementation:dendrogram:worst}, this computation would take $\frac{1}{2} n(n+1) - 1$ operations to complete, what is a bit more than ${n \choose 2}$ operations needed to compute the pairwise distance matrix. On the other hand, in the best case where the $n-1$ nodes would form a perfect binary tree, as pictured in \autoref{fig:implementation:dendrogram:best}, only $O(n(log)n)$ DTW projection computations would be needed.

Now, in the worst-case, the phenomenon of \emph{chaining} would force all of these $n-1$ clusters to line-up into $n-1$ distinct levels, as in \autoref{fig:implementation:dendrogram:worst}. 

\subsection{Explorer module}
