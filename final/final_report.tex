...
After splicing the resulting messenger RNA (mRNA) is transferred to ribosome, where the proteins are created from the blueprint encoded inside the mRNA. A more in depth view of these processes can be found in a molecular biology textbook, e.g. \cite{Alberts:2002vj,Alberts:2002wf}.

It is known that the physical structure of DNA plays an important part in gene regulation. 
Tightly packed regions of DNA are harder to interact with, therefore harder for transcription factors or RNA polymerase to bind with the DNA. Modifications of the core histones, can change the structure of chromatin and therefore change the acessiblity of DNA for processes such as transcription.
For instance, acetylation of histone H4 on lysine 16 (shortened as \histonemodification{H4K16Ac}) has been shown to inhibit formation of the 30-nanometer fiber (panel 5 in \ref{fig:background:dna_packaging})\cite{ShogrenKnaak:2006gt}. Other histone modifications, such as Histone 3 lysine 4 trimethylation (\histonemodification{H3K4Me3}) have been shown to locate near transcription start sites and correlate with the transcriptional activity of genes\cite{Pokholok:2005wn}.
