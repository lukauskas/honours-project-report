...
Packages in PyLab, namely \verb"numpy", \verb"scipy" and \verb"matplotlib" also play a very important role. Together these packages bring functionality of MATLAB\footnote{See \url{http://www.mathworks.co.uk/products/matlab/} for more information about MATLAB} to Python. The first package, \verb"numpy" provides efficient computational routines on arrays and matrices; the second package, \verb"scipy" has a majority of scientific procedures implemented, including the ones required for drawing and analysing dendrograms; whereas the third one, \verb"matplotlib" \cite{Hunter:2007ux}, handles all of the visualisation and plotting. \todo{Get a link when scipy.org is back}

Daniel M\"ullner's \verb"fastcluster" package \cite{Mullner:2011wb}, is used for efficient computations
of hierarchical clustering linkage matrices; and finally, \verb"pysam" (\url{https://code.google.com/p/pysam/}) provides a Python wrapper around the \verb"samtools" (\cite{Li:2009ka}) toolkit.
